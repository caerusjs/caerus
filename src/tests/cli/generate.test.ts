...
test('generating a resource', (done) => {
  childProcess.exec('yarn caerus g resolver test', (_, stdout) => {
    // Check we have the correct console message
    expect(stdout).toMatch(/.*Created file.*\/server\/src\/resolvers\/test.resolver.ts/)
    expect(stdout).toMatch(/.*Created file.*\/server\/src\/resolvers\/test.test.ts/)
    expect(stdout).toMatch(/.*Created file.*\/server\/src\/resolvers\/test.input.ts/)
    expect(stdout).toMatch(/.*Created file.*\/server\/src\/resolvers\/test.args.ts/)
    done()

    // Check out entity class name is correct
    const resolverContents = fs.readFileSync(`${process.cwd()}/server/src/resolvers/test.resolver.ts`, { encoding: 'utf8' })
    expect(resolverContents).toMatch(/.*export class TestResolver/)

    const testContents = fs.readFileSync(`${process.cwd()}/server/src/resolvers/test.test.ts`, { encoding: 'utf8' })
    expect(testContents).toMatch(/.*test\('getTest', async \(\) => {/)

    const inputContents = fs.readFileSync(`${process.cwd()}/server/src/resolvers/test.input.ts`, { encoding: 'utf8' })
    expect(inputContents).toMatch(/.*export class AddTestInput implements Partial<Test>/)

    const argsContents = fs.readFileSync(`${process.cwd()}/server/src/resolvers/test.args.ts`, { encoding: 'utf8' })
