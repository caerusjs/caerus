...
export const createCache = (name: string, action: 'AddRemove') => {
  switch (action) {
    case 'AddRemove':
      return (`import { Get${titleize(name)}sQuery, Get${titleize(name)}sDocument } from 'types/graphql';
...
export const write${titleize(name)}sCache = ({ cache, data }: Write${titleize(name)}sCache) =>
