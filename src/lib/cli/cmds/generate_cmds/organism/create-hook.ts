...
const useGet${titleize(name)} = (id: string) => {
  return useQuery<Get${titleize(name)}Query, Get${titleize(name)}QueryVariables>(Get${titleize(name)}Document, {
    variables: {
      id: id,
    },
  });
...
const useAdd${titleize(name)} = () => {
  return useMutation<Add${titleize(name)}Mutation, Add${titleize(name)}MutationVariables>(
    Add${titleize(name)}Document,
    {
      update(cache, { data }) {
        ...
        if (dataCache?.get${titleize(name)}s && data?.add${titleize(name)}) {
          const updatedData = [data.add${titleize(name)}].concat(dataCache.get${titleize(name)}s);
...
const useUpdate${titleize(name)} = () => {
  return useMutation<Update${titleize(name)}Mutation, Update${titleize(name)}MutationVariables>(
    Update${titleize(name)}Document,
  );
};
